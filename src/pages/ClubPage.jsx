...
// ── DESIGN TOKENS — dark slate + baby blue ────────────────────────────────────
const P = {
  pageBg:      '#111620',
  cardBg:      '#1A1F2E',
  ...
  accent:      '#5BA8D8',        // baby blue
  accentDim:   'rgba(91,168,216,0.50)',
  accentGlow:  'rgba(91,168,216,0.22)',
  textPri:     '#D8E8F4',        // primary text
  textMid:     '#7A96AE',        // secondary
  textDim:     '#46606E',        // captions
  hoopOrange:  '#D9801A',
  white:       '#FFFFFF',
  swapRing:    '#63E2FF',        // highlight colour for swap mode
}
...
// % position of each slot on the 420px court — guards top, bigs bottom
const COURT_POS = {
  PG: { x: '26%', y: '11%' },
  SG: { x: '74%', y: '11%' },
  SF: { x: '16%', y: '40%' },
...
const SLOT_SIZE = 88   // all positions same size

function loadClub()       { try { return JSON.parse(localStorage.getItem(STORAGE_KEY)) } catch { return null } }
function saveClub(data)   { localStorage.setItem(STORAGE_KEY, JSON.stringify(data)) }

// ── COURT SVG (dark slate floor, baby-blue lines, amber hoop) ─────────────────
  ...
  return (
    <svg viewBox="0 0 360 420" preserveAspectRatio="xMidYMid slice"
      ...
      <g clipPath="url(#cClip)">
        ...
        {/* Hash marks */}
        {[[118,318],[118,349]].map(([x,y]) => (
          <line key={`${x}${y}`} x1={x} y1={y} x2={130} y2={y}
            stroke="rgba(88,160,215,0.25)" strokeWidth="1.2"/>
        ))}
        {[[230,318],[230,349]].map(([x,y]) => (
          <line key={`r${x}${y}`} x1={x} y1={y} x2={242} y2={y}
            stroke="rgba(88,160,215,0.25)" strokeWidth="1.2"/>
        ))}
        ...
        <circle cx="180" cy="397" r="13"
          fill="none" stroke={P.hoopOrange} strokeWidth="2.2"/>
...
function PlayerSlot({ position, member, onPress, swapMode, isSwapSource }) {
  ...
  const ringColor = isSwapSource ? P.swapRing
    : member ? 'rgba(88,160,215,0.60)' : P.accentDim
        ...
        {member ? (
          ...
          <>
            ...
            <line x1="45" y1="34" x2="45" y2="56"
              stroke={swapMode ? 'rgba(99,226,255,0.50)' : P.accent} strokeWidth="2.8" strokeLinecap="round"/>
        ...
        {member ? (
          <>
            ...
            <button onClick={onRemove} style={{
              ...
              borderRadius: 12, color: '#F87171', fontFamily: 'var(--font-display)',
...
function CreateClub({ onCreated, ownerName, ownerInitial }) {
  ...
  return (
    <div style={{ minHeight: '100%', display: 'flex', flexDirection: 'column',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        {[
          { label: 'Nazwa klubu', ph: 'np. Hoopconnect Warsaw', val: name,
            onChange: e => setName(e.target.value), max: 30, style: { fontSize: 15 } },
          { label: 'Skrót (2–3 litery)', ph: 'HCW', val: abbr,
            onChange: e => setAbbr(e.target.value.toUpperCase().replace(/[^A-Z]/g,'').slice(0,3)),
            max: 3, style: { fontSize: 22, fontWeight: 800, letterSpacing: 5,
              fontFamily: 'var(--font-display)' } },
        ].map(f => (
          <div key={f.label}>
            <p style={{ fontSize: 9, fontWeight: 700, letterSpacing: 2,
              textTransform: 'uppercase', color: P.textDim, marginBottom: 6 }}>
              {f.label}
            </p>
            <input placeholder={f.ph} value={f.val} onChange={f.onChange} maxLength={f.max}
              style={{ width: '100%', padding: '13px 14px',
                ...
                fontFamily: 'inherit', boxSizing: 'border-box', ...f.style }}/>
          </div>
        ))}
      ...
      <motion.button whileTap={{ scale: 0.97 }} onClick={handleCreate} disabled={!valid}
        style={{ marginTop: 24, width: '100%', padding: '16px',
          background: valid ? `linear-gradient(135deg, ${P.accent}, #1A6AB0)` : P.cardBg,
          ...
          color: valid ? P.white : P.textDim, cursor: valid ? 'pointer' : 'default',
          transition: 'all 0.2s',
          boxShadow: valid ? 'rgba(43,108,176,0.40) 0 6px 24px' : 'none' }}>
        Utwórz Klub
      </motion.button>
    </div>
  )
...
// ── CLUB PANEL ────────────────────────────────────────────────────────────────
function ClubPanel({ club, onUpdate, ownerName }) {
  const [invite,     setInvite]     = useState(null)
  const [swapMode,   setSwapMode]   = useState(false)
  const [swapSource, setSwapSource] = useState(null)   // position key
  const [copiedClub, setCopiedClub] = useState(false)

  const isOwner = club.ownerName === ownerName
    ...
    if (swapMode && isOwner) {
      if (!swapSource) {
        ...
        if (swapSource !== position.key) {
          const a = club.members[swapSource]
          const b = club.members[position.key]
          const updated = {
            ...club,
            members: { ...club.members, [swapSource]: b, [position.key]: a },
          }
          saveClub(updated)
          onUpdate(updated)
        }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100%',
      ...
      <AnimatePresence>
        {invite && (
          <InviteModal club={club} position={invite.position} member={invite.member}
      ...
      <div style={{ padding: '22px 16px 0' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 18 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <p style={{ fontSize: 12, color: P.textMid, marginTop: 4, display: 'flex',
        ...
        <AnimatePresence>
          {swapMode && (
            <motion.div
              ...
              <p style={{ fontSize: 11, color: P.swapRing, fontWeight: 600,
                textAlign: 'center', padding: '8px 16px',
                background: 'rgba(99,226,255,0.07)',
                border: `1px solid rgba(99,226,255,0.20)`,
                borderRadius: 10 }}>
                {swapSource
      ...
      <AnimatePresence mode="wait">
        {!club ? (
          ...
          <motion.div key="panel"
            ...
            <ClubPanel club={club} onUpdate={setClub} ownerName={ownerName}/>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
